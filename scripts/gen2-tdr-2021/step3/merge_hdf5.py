...
def merge2(filenames, output_filename):
    ...
    for f in filenames:
        ...
        if '.tar.gz' in f:
            ...
            for member in tar.getmembers():
                if member.isfile():
                    ...
                    fin = h5py.File(member.name, 'r', driver='core') # load everything into memory
                    os.remove(member.name) # delete the file to prevent disk bloat
        else:
        ...
        # print("Usage {} MB".format(usage/1e3))
    ...
    if(len(non_empty_filenames)):
        ...
        f = filenames[0]
        print("Going to try and use file {} as the basis for event tracking".format(f))
