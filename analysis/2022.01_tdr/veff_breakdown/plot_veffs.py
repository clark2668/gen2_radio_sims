...
for flavor in flavors:
    ...
    pkl_file_name = os.path.join('results/overlap_' + detector + "_deeptrig_" + deep_trigger + "_shallowtrig" + shallow_trigger + "_" + flavor + ".pkl")
    ...
    for lgE, value in coincidences.items():
        ...
        veff_deep = value['deep_only_veff']
        ...
        result[flavor]['deep_veff'].append(veff_deep)
        ...
        result[flavor]['deep_aeff'].append(aeff_deep)
...
for iflavor, flavor in enumerate(flavors):
    for iE, energy in enumerate(energies):
        ...
        average_deep_veff[iE]+=result[flavor]['deep_veff'][iE]/3
        ...
        average_deep_aeff[iE]+=result[flavor]['deep_aeff'][iE]/3
...
output_csv = 'log10(energy [eV]), veff total [km^3 sr water equiv], veff deep only, veff shallow only, veff coincidence, '
output_csv += 'aeff total [km^2 sr water equiv], aeff deep only, aeff shallow only, aeff coincidence, '
output_csv += 'frac deep only, frac shallow only, frac coincidence'
output_csv += "\n"
for iE, energy in enumerate(energies):
...
axs[0].plot(xx, average_deep_aeff, styles[0], label='Deep Component')
axs[0].plot(xx, average_shallow_aeff, styles[1], label='Shallow Component')
axs[0].plot(xx, average_dual_aeff, styles[2], label='Deep-Shallow Coincidence')
...
axs[1].plot(xx, fraction_deep, styles[0], label='Deep Component')
axs[1].plot(xx, fraction_shallow, styles[1], label='Shallow Component')
axs[1].plot(xx, fraction_dual, styles[2], label='Deep-Shallow Coincidence')
...
outfilename = f'plots/veff_fractions_{detector}_deeptrig_{deep_trigger}_shallowtrig_{shallow_trigger}.png'
