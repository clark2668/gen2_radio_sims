...
for flavor in flavors:
	file = "step2_" + depth + 'm_' + flavor + "_stats.txt"
	with open(file) as fp:
		...
		while line:
			...
			czmin = split_type[6] + '.' + split_type[7]
...
for flavor in flavors:
	...
	if flavor=='e':
		...
		ax.set_ylim([0,350])
	ax.set_title('time per step 0 event for nu-{}'.format(flavor),size=15)
	ax.tick_params(labelsize=15)
	ax.legend()
	fig.savefig("time_per_event_{}m_{}.png".format(depth, flavor),edgecolor='none', bbox_inches='tight')
