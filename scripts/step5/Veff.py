...
def remove_duplicate_triggers(triggered, gids):
    ...
    # print("Usage in MB is {}".format(usage/1e3))

    # print("Num gids is {}".format(len(uids[counts>1])))
...
def get_Veff_Aeff_single(filename, trigger_names, trigger_names_dict, trigger_combinations, deposited, station, veff_aeff="veff"):
    ...
    if(triggered.size == 0):
        ...
        for iT, trigger_name in enumerate(trigger_names):
            if (not 'PA' in trigger_name) and (not 'LPDA' in trigger_name):
                print("skipping trigger {}".format(trigger_name))
                continue

            print("Working on trigger {}".format(trigger_name))
